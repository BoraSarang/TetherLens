...
| ID | Feature | Description |
|----|---------|-------------|
| FR-13 | **앱별 트래픽 모니터링** | nettop 기반 프로세스별 트래픽 순위 (Network Extension 미채택) — ✅ v0.x 구현 |
| FR-14 | **앱별 차단/허용** | 절약 모드 시 시스템 작업 중지 + `/etc/hosts` 차단. **per-app 패킷 차단은 보류** — `NEFilterDataProvider`는 Apple 유료 개발자 계정 + 시스템 확장 필요로 무료/OSS 배포와 충돌 |
