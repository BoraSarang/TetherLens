...
### Added
- **macOS 업데이트 시스템** `[macOS]` (bd 60h) — GitHub Releases 기반 개인 배포 업데이트. 실행·팝오버 열 때 주기(매 실행/매일/매주/안함, 기본 매주)에 맞춰 조용히 확인, 새 버전 시 전용 창에 릴리스 노트(마크다운 블록 렌더) + 설치 방법 + 다운로드(릴리스 페이지). 설정 탭(버전 표시·확인 주기·수동 확인) + 팝오버 "..."·우클릭 더보기·⌘⇧P "업데이트 확인". 404(릴리스 0개) 별도 구분. 마지막 확인 시각 UserDefaults 영속화. `release.yml` — v태그 푸시 → 버전 일치 검증 → 테스트 → Release 빌드 → 번들 → ad-hoc 서명 → Zip → Release 자동 발행
...
- **메뉴바 호버 툴팁** — `{SSID} · ▲업 ▼다운 · QoS n%` 1줄 상태 표시 (클릭 없이 확인)

### Changed (Phase A — 통계 의미 개선)
- **차트 누적선 제거** — 단조증가 누적 LineMark가 Y축을 누적 스케일로 잡아먹어 일별 막대를 찌그러뜨리던 문제 해소. Y축은 막대 기준으로 복원, `cumulativeTotal` 삭제
- **평균 페이스 점선 추가** — 최근3일 평균 RuleMark(accent 점선 + "평균" 라벨). 평균 초과일이 한눈에 구분됨
...
- **상위 프로세스 Top5** — 앱 아이콘(`AppIconResolver`, NSWorkspace+번들 매칭, 캐시) + 업/다운 속도
...
- **인사이트 카드→히어로** — 28pt 합산 + 증감 + QoS 게이지 + Top 3행(도트, 드릴다운 유지)
- **앱 탭 16px 아이콘 + 행 호버** — 상세·세션·앱 테이블 행 하이라이트
- **히트맵 적응형 셀** — `GeometryReader`로 창폭 맞춤, 가로 스크롤 제거. 차트·히트맵 하단 `Spacer` 제거

### Changed (순서 통일 — 업로드 좌 / 다운로드 우)
- **팝오버 대형 숫자·Top5 값 열·헤더점·메뉴바 툴팁** 순서 교체 (다른 화면은 이미 업 우선이라 3곳만 변경)

### Changed (플로팅 프리미엄화)
- **상태점 + 디바이더 + 앱 아이콘** — Top3 행에 14px 아이콘, 속도 하단 구분선, 좌상단 도달성 도트
- **호버 투명도 슬라이더** — 하단 오버레이로 직접 조절 (설정과 동일 키·범위 35~100%, 레이아웃 불변)
- **높이 120→132** (아이콘·디바이더 공간 확보)

### Changed (리포트 대시보드화)
- **인사이트→스탯 카드 4장** — 합산/업/다운/평균 (아이콘+라벨+값, `cardBackground` 토큰). 증감률은 차트 헤더로 이식
- **차트 카드화** — 카드 배경 + X축 라벨 공간 확보(304) + 범례 간격. 평균·할당량 점선은 막대 뒤로
- **테이블 카드화 + 앱 행 점유율 바** — 상세·세션·앱 테이블 카드 배경, 앱 행에 섹션 내 비율 바
- **사이드바 섹션 헤더** — 보기(그래프/상세/세션) + 분석(시간대별/앱/리포트)

### Fixed
- **플로팅 상단 여백 + 이중 디바이더** — 상단 행 좌우 12pt·상단 8pt, `trafficSection` 잔재 구분선 삭제 (높이 132 유지)
- **설정 5탭·리포트 본문 상단 정렬** — Form 중앙 뜸 해소 (`maxHeight top` 핀). 가로는 Form 기본 leading 유지
